...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  $raw_name = trim(htmlspecialchars($_POST["name"])); // transform any html characters into html entities 
  ...
  if (in_array($raw_character, ["wizard", "mage", "orc", "bird"])) {
  ...
  $options = ["options" => ["min_range" => 1900, "max_range" => date("Y")]]; // filter_var to limit the range of years that users can submit
  } else {
...
Select a character:
  ...
  <input type="radio" name="character" value="bird" <?php echo ($character=='bird')?'checked':'' ?>> Bird
